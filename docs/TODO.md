...
21. 修复 Profile 磁贴拖拽虚影并正式接入头像图片：拖拽改为 pointer-only，头像资源稳定服务自 `public/assets/Avatar.jpg`。
22. 完成 BGM 播放器功能升级：接入三曲目数据结构、播放/暂停、循环切歌、进度、独立竖向音量、GSAP 展开动效、错误回退和本地状态持久化。
23. 完成 BGM 自动化测试、生产构建与桌面/平板 Rendered State 验收；正式音频与封面素材及移动端浏览器复验仍待补齐。
24. 接入三首正式 BGM 与真实元数据，控制按钮改为 SVG 图标，进度/音量改为线状轨道，并以裁切导轨重构详情抽屉；完成桌面、平板、手机和 `360px` Rendered State 复验。
25. 收敛 BGM 最终细节：默认 10% 自动播放尝试、音量条自绘黑轨/粉线/粉心黑边滑块、音量百分比文本、封面与右侧时间对齐、细化非主按钮边框，并修复收起时底卡圆角提前恢复。
26. 优化 BGM 手机端细节：移除 `tap play` 可见提示，音量按钮与进度轨居中对齐并避开底卡边界，卡片收窄至 `178px`，完成 `390px` 与 `360px` 收起/展开 Rendered State 复验。
27. 修复 BGM 展开态三个传输按钮错位：改用明确的左、中、右锚点替代不对称 transform 位移，并复验桌面与手机端等距排列。

## 下一步任务

1. 补齐 BGM 的三张正式专辑封面，并替换当前 `NO ART` 回退槽位。
