...
- 手风琴 hover 使用近白底和对应板块色文字；不保留当前页面高亮状态，不使用 underline、阴影或悬浮位移。
...
- `SidebarAccordion` 的 hover 视觉反馈由 `.sidebar-band::before` 和 `.mobile-menu-band::before` 绘制近白色整条背景，文字切换为对应板块色；不使用 `.is-active` 当前页高亮。
