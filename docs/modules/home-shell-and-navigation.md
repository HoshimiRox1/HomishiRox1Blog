...
- 视觉系统已切换为冷白底、黑色粗边框、密网格背景和淡紫 / 浅青 / 奶黄 / 嫩绿配色；Home、Profile、Portfolio、Blog 的 hover 文案颜色分别来自各自的深色强调色。
...
- `CharacterStage` 的粉色舞台壳、`CHARACTER` 标签和 `ROXY STAGE` 面板常驻首屏；角色图和人物 meta 仅在 Home 首次滚轮后入场。人物 meta 只显示角色名和绿色 meter 条，不显示角色描述文本，避免遮挡人物脸部。每个角色保留独立的 figure/img 节点，角色切换前会清理旧 GSAP tween、隐藏所有角色 figure，再让当前角色自己的节点入场，避免复用同一个 `<img src>` 时出现上一角色旧帧闪烁。
